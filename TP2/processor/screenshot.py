...
def take(url: str) -> bytes:
    ...
    La imagen se guarda en el directorio 'screenshots' relativo al proyecto
    y luego se devuelve como bytes.
    ...
    try:
        ...
        # --- Guardado de la Imagen (PATH RELATIVO) ---
        # Usamos Path para hacerlo multiplataforma y relativo al directorio del proyecto
        project_root = Path(__file__).parent.parent  # TP2/
        ...
        print(f"[Screenshot] Captura para {url} completada y leída ({len(screenshot_bytes)} bytes).")
        return screenshot_bytes

    except TimeoutException as e:
        print(f"[ERROR][Screenshot] Timeout al cargar {url}: {e}")
